...
- Fyne’s test driver runs `fyne.Do` inline. Use `dropAndWait`, `waitFor*`, feature `Settle`, and existing completion channels before assertions; never sleep to guess completion. Grid, display, comparison, mosaic window, deletion, slideshow, EXIF, Spiral, Explorer, visual search, and analysis-cache work marshal through the `UIQueue` exception above.
...
- **HEIC/HEIF support is disabled** pending decoder distribution and security qualification. See [the removal record](plans/2026-09-14-remove-heic-decoder.md) before considering restoration.
