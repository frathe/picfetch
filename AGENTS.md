...
- `internal/ui/display.Feature` owns the single-image surface, captures, rotation/fades, GIF/SVG work, and complete navigation/preload lifetimes. Root injects its shared image cache, chooses sources/retries/neighbors and supplies UI callbacks. Zoom owns surface geometry; display alone publishes pixels. Read `Snapshot()` or capture content rather than coordinate mutable frames.
...
- Input flows through CLI/open/drop → `handleDrop` scan → `filesort.Order` → `ShowImage` → `display.Load` → `internal/imaging` probe/decode/orient/cache → display surface. Reuse this path rather than creating parallel open/load logic. On macOS, "Open With"/Dock drop/`open -a`/double-click arrive as an Apple Event instead of argv; `internal/openwith`'s queue + `internal/ui/openwith.go` fold that into the same `handleDrop` call, argv files first.
- `internal/imaging` is viewer-independent. Full images and grid thumbnails use separate byte-budgeted caches; preserve `ByteCache.Add` foreground admission, which can retain an oversized displayed image. Display preloads use generation-bound `CacheWriter.AddIfRoom` for atomic admission without eviction or promotion. `AddIfFits` remains size-bounded but may evict, and retains that behavior for its other callers.
...
- Display dispatches GIF frames through its instance `UIQueue`, with one buffered acknowledgement before the next delay. A cancelled worker stops without UI drain; queued callbacks recheck the animation identity. Stable capture owns an idempotent acquisition-bound release. Picture-frame advances use the same acknowledgement rule on their owning `UIQueue`; Exit/Close releases the worker before `Settle` drains stale callbacks.
- Display `LoadDone` covers its complete retry chain and root handoff and GIF/preload admission; it does not mean playback or speculation has stopped. `AppliedFrames`, `AnimationDone` and `WaitPreloads` observe those distinct effects. `Settle` joins finite load/SVG/preload work, drains UI and repeats; it never waits for continuous GIF playback. `Clear` permits reopening; `Stop` closes admission. Both cancel without UI waits. After Stop, `Wait` joins all retired generations before stale UI delivery is drained.
...
- Fyne’s test driver runs `fyne.Do` inline. Use `dropAndWait`, `waitFor*`, feature `Settle`, and existing completion channels before assertions; never sleep to guess completion. Grid, display, comparison, mosaic window, deletion, slideshow, EXIF, Spiral, Explorer, visual search, analysis-cache, and release-note artwork marshal through the `UIQueue` exception above.
...
- Release-note artwork uses Help's per-instance UIQueue. HTTP fetch and image
  decoding run on tracked workers; window close cancels its session, and queued
  results recheck cancellation before touching pixels. Stop ends admission
...
- Complete Docker suites require a native Linux/amd64 daemon, checked by `make check-test-platform` before setup. ARM-hosted amd64 emulation can reject the worker seccomp filter; never skip isolation tests or relax worker policy to turn that into a clean gate. Run complete verification from a native amd64 checkout or use native amd64 CI. `make golden` and `make check-test-shards` remain available under emulation.
...
- Desktop associations and portable format rules use `imaging.RecognizedExtensions()`; `SupportedExtensions()` describes unconditional decoders. Optional HEIC availability comes from the captured `heic.Capability`, never the build host's installed codecs.
...
- **HEIC/HEIF uses system decoders in bounded workers.** Preserve captured capability generations and normal image-consumer paths. Let the native decoder render color; ICC/gamut/HDR fidelity is best effort, not an admission gate. See [the implementation plan](plans/2026-09-22-system-heic.md) for platform qualification gaps and [the removal record](finished_refactorings/2026-09-14-remove-heic-decoder.md) before changing decoder distribution.
