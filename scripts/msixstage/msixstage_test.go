...
func TestStoreWorkflowPublishingContract(t *testing.T) {
	...
	prepare, publish, found := bytes.Cut(publisher, []byte("\n  publish:"))
	if !found {
	...
	actions := regexp.MustCompile(`(?m)^\s+(?:-\s+)?uses:\s+(\S+)`).FindAllSubmatch(publish, -1)
	if len(actions) == 0 {
		t.Fatal("protected publish job missing action references")
	}
	pinnedAction := regexp.MustCompile(`^[^@]+@[0-9a-f]{40}$`)
	for _, action := range actions {
		if !pinnedAction.Match(action[1]) {
			t.Errorf("protected publish action must use an immutable full commit SHA: %s", action[1])
		}
	}
}
