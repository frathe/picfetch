...
- [ ] All three paths construct equivalent file-size/EXIF facts for the same encoded bytes, including a GPS JPEG.
- [ ] Comparison-first normal navigation exposes the real size and EXIF affordance in the actual UI tree.
- [ ] Preserve EXIF-corrected pixels, RAW preview markers, original animation frames, encoded-size limits and stale-request checks.
- [ ] Preserve explicit foreground Add versus speculative AddIfFits behavior and separate thumbnail/full-image cache budgets.
