...
- [ ] Pause old reads across source replacement/reset and release both success and failure; neither hash nor thumbnail paths change new-generation facts.
- [ ] Check the captured generation atomically with every model mutation, before a later UI installation guard can become relevant.
- [ ] Cover reorder/adoption separately so valid established facts can survive without adopting facts from replacement sources.
- [ ] Exercise native-dimension backfill and failure retry behavior, with deterministic gated workers and observable completion.
