...
void main() {
    vec2 pixel = vec2(gl_FragCoord.x - paneX, (frame.y - gl_FragCoord.y) - paneY);
    vec4 color = vec4(0.0);
    float bestStep = 0.0;

    if (min(tile0StepX, tile0StepY) > 0.0 && (bestStep == 0.0 || max(tile0StepX, tile0StepY) < bestStep) && tileContains(pixel, tile0MinX, tile0MinY, tile0StepX, tile0StepY, tile0Width, tile0Height)) {
        color = texture2D(detail0, tileCoordinate(pixel, tile0MinX, tile0MinY, tile0StepX, tile0StepY, tile0Width, tile0Height));
        bestStep = max(tile0StepX, tile0StepY);
    }
    if (min(tile1StepX, tile1StepY) > 0.0 && (bestStep == 0.0 || max(tile1StepX, tile1StepY) < bestStep) && tileContains(pixel, tile1MinX, tile1MinY, tile1StepX, tile1StepY, tile1Width, tile1Height)) {
        color = texture2D(detail1, tileCoordinate(pixel, tile1MinX, tile1MinY, tile1StepX, tile1StepY, tile1Width, tile1Height));
        bestStep = max(tile1StepX, tile1StepY);
    }
    if (min(tile2StepX, tile2StepY) > 0.0 && (bestStep == 0.0 || max(tile2StepX, tile2StepY) < bestStep) && tileContains(pixel, tile2MinX, tile2MinY, tile2StepX, tile2StepY, tile2Width, tile2Height)) {
        color = texture2D(detail2, tileCoordinate(pixel, tile2MinX, tile2MinY, tile2StepX, tile2StepY, tile2Width, tile2Height));
        bestStep = max(tile2StepX, tile2StepY);
    }
    if (min(tile3StepX, tile3StepY) > 0.0 && (bestStep == 0.0 || max(tile3StepX, tile3StepY) < bestStep) && tileContains(pixel, tile3MinX, tile3MinY, tile3StepX, tile3StepY, tile3Width, tile3Height)) {
        color = texture2D(detail3, tileCoordinate(pixel, tile3MinX, tile3MinY, tile3StepX, tile3StepY, tile3Width, tile3Height));
        bestStep = max(tile3StepX, tile3StepY);
    }
    if (min(tile4StepX, tile4StepY) > 0.0 && (bestStep == 0.0 || max(tile4StepX, tile4StepY) < bestStep) && tileContains(pixel, tile4MinX, tile4MinY, tile4StepX, tile4StepY, tile4Width, tile4Height)) {
        color = texture2D(detail4, tileCoordinate(pixel, tile4MinX, tile4MinY, tile4StepX, tile4StepY, tile4Width, tile4Height));
        bestStep = max(tile4StepX, tile4StepY);
    }
    if (min(tile5StepX, tile5StepY) > 0.0 && (bestStep == 0.0 || max(tile5StepX, tile5StepY) < bestStep) && tileContains(pixel, tile5MinX, tile5MinY, tile5StepX, tile5StepY, tile5Width, tile5Height)) {
        color = texture2D(detail5, tileCoordinate(pixel, tile5MinX, tile5MinY, tile5StepX, tile5StepY, tile5Width, tile5Height));
        bestStep = max(tile5StepX, tile5StepY);
    }
    if (min(tile6StepX, tile6StepY) > 0.0 && (bestStep == 0.0 || max(tile6StepX, tile6StepY) < bestStep) && tileContains(pixel, tile6MinX, tile6MinY, tile6StepX, tile6StepY, tile6Width, tile6Height)) {
        color = texture2D(detail6, tileCoordinate(pixel, tile6MinX, tile6MinY, tile6StepX, tile6StepY, tile6Width, tile6Height));
    ...
    if (bestStep == 0.0) {
        ...
        color = texture2D(overview, local);
    }
...
func newShaderPaneRenderer(index int) paneRenderer {
	...
	shader.Uniforms = make(map[string]float32, 2+detailSamplerCount*6)
	shader.Uniforms["paneX"] = 0
	shader.Uniforms["paneY"] = 0
...
func (r *shaderPaneRenderer) Present(scene paneScene) {
	...
	r.scene = scene
	r.shader.Uniforms["paneX"] = float32(scene.panePosition.X)
	r.shader.Uniforms["paneY"] = float32(scene.panePosition.Y)
...
func (r *shaderPaneRenderer) bindDetail(slot int, tile *renderTile, scene paneScene) {
	if slot < 0 || slot >= detailSamplerCount || tile == nil || tile.texture == nil ||
		scene.source == nil || scene.source.frame == nil ||
		scene.imageSize.Width <= 0 || scene.imageSize.Height <= 0 ||
		scene.displaySize.X <= 0 || scene.displaySize.Y <= 0 {
		return
	}
	sourceSize := scene.source.frame.Bounds().Size()
	if sourceSize.X <= 0 || sourceSize.Y <= 0 {
		return
	}
	pointToPixelX := float64(scene.displaySize.X) / float64(scene.imageSize.Width)
	pointToPixelY := float64(scene.displaySize.Y) / float64(scene.imageSize.Height)
	sourceToPixelX := float64(scene.displaySize.X) / float64(sourceSize.X)
	sourceToPixelY := float64(scene.displaySize.Y) / float64(sourceSize.Y)
	stepX := float64(tile.scale) * sourceToPixelX
	stepY := float64(tile.scale) * sourceToPixelY
	minX := float64(scene.imagePosition.X)*pointToPixelX + float64(tile.interior.Min.X)*stepX
	minY := float64(scene.imagePosition.Y)*pointToPixelY + float64(tile.interior.Min.Y)*stepY
	r.bound[slot] = tile
	r.shader.Textures[detailTextureName(slot)] = tile.texture
	r.shader.Uniforms[detailUniform(slot, "MinX")] = float32(minX)
	r.shader.Uniforms[detailUniform(slot, "MinY")] = float32(minY)
	r.shader.Uniforms[detailUniform(slot, "StepX")] = float32(stepX)
	r.shader.Uniforms[detailUniform(slot, "StepY")] = float32(stepY)
