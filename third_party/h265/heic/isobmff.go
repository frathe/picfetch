...
// reserve accounts for parsed objects, including zero-width iloc extents.
// Payload size alone cannot bound the number of such objects.
func (m *metaBox) reserve(n int) bool {
	if n < 0 || n > maxSamples-m.entries {
		return false
	}
	m.entries += n
	return true
...
func (m *metaBox) data(it *item, src *source) ([]byte, error) {
	...
	span := func(e extent) (uint64, uint64, error) {
		if it.baseOffset > src.size || e.off > src.size-it.baseOffset {
		...
		if off > src.size || n > src.size-off || n > maxItemBytes {
			return 0, 0, ErrInvalid
		}
	...
	// Validate all spans before reading or allocating. Repeated extents must
	// not amplify a small source into an arbitrarily large allocation.
	var total uint64
	for _, e := range it.extents {
		...
		if n > min(src.size, maxItemBytes)-total {
			return nil, ErrInvalid
		}
		total += n
	}
	...
	for _, e := range it.extents {
		off, n, _ := span(e) // Validated above against the same immutable source.
...
func parseHvcC(r *reader) (*hevcConfig, error) {
	...
	for range arrays {
		...
		n := int(r.u16())
		if r.err || n > r.remaining()/4 || n > 1024-len(c.paramSets) {
			return nil, ErrInvalid
		}
