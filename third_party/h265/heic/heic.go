...
// Resource ceilings apply to every entry point, including metadata readers.
// The per-item ceiling also bounds one HEVC access unit or metadata payload.
const (
...
func (f *file) setOptions(o Options) {
	...
	f.threads = o.Threads
	if n := f.limit(); n > 0 {
		// Covers edge tiles and alpha without allowing repeated cropped
		// pictures or non-output sequence frames to multiply decode work.
		f.budget = hevc.NewDecodeBudget(int64(n) * 4)
	}
}
...
func (f *file) decodeItem(dec *itemDecoder, it *item) (*hevc.Picture, error) {
	// The coded SPS may disagree with the container's ispe. Enforce the
	// budget in HEVC before it allocates pictures, including tiles and alpha.
	dec.d.FrameSizeLimit(f.limit())
	dec.d.Budget(f.budget)
	if it.typ == "grid" {
		return nil, ErrUnsupported
	}
	...
	frames := 0
	err = eachNAL(data, cfg.hvcC.lengthSize, func(u hevc.NALUnit) error {
		if u.Type.IsVCL() && len(u.RBSP) > 0 && u.RBSP[0]&0x80 != 0 {
			frames++
			// An image item is one picture; refuse an appended sequence before
			// allocating its second frame.
			if frames > 1 {
				return ErrInvalid
			}
		}
...
// FrameSizeLimit also bounds the aggregate coded pixel area of each track.
...
// DecodeConfigWithOptions returns the display dimensions after the requested
// container transforms and applies the same pixel budget as Decode. Coded SPS
// dimensions are independently checked when Decode reaches the image data.
func DecodeConfigWithOptions(r io.Reader, o Options) (image.Config, error) {
	f, err := parseHeader(r)
	if err != nil {
		return image.Config{}, err
	}
	...
	if n := f.limit(); n > 0 && uint64(cfg.Width)*uint64(cfg.Height) > uint64(n) {
		return image.Config{}, ErrUnsupported
	}
	if o.AutoRotate {
		if it, err := f.primary(); err == nil {
			if clap := f.meta.prop(it, "clap"); clap != nil {
				_, _, w, h, ok := cropRect(&clap.clap, cfg.Width, cfg.Height)
				if !ok {
					return image.Config{}, ErrInvalid
				}
				cfg.Width, cfg.Height = w, h
			}
			if rot := f.meta.prop(it, "irot"); rot != nil && rot.angle&1 != 0 {
