...
func parseVPS(rbsp []byte) (*vps, error) {
	...
	for i := start; i <= int(v.maxSubLayersMinus1); i++ {
		...
		if v.maxDecPicBuffering[i] > 15 || v.maxNumReorderPics[i] > v.maxDecPicBuffering[i] {
			return nil, ErrInvalid
		}
	}
...
func parseSPS(rbsp []byte) (*sps, error) {
	...
	for i := start; i <= int(s.maxSubLayersMinus1); i++ {
		...
		// HEVC's decoded-picture buffer holds at most 16 pictures; the
		// syntax element stores that capacity minus one.
		if s.maxDecPicBuffering > 15 || s.maxNumReorderPics > s.maxDecPicBuffering {
			return nil, ErrInvalid
		}
	}
