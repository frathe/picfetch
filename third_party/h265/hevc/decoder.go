...
func (d *Decoder) decodeSlice(nal NALUnit) ([]*Picture, error) {
	...
	// 7.4.3.2.1 activates one pair of parameter sets for a whole picture, so a
	// set re-sent between its slice segments describes the next one.
	if !first && d.ctu != nil {
		if p.id != d.ctu.p.id {
			return nil, ErrInvalid
		}
	...
	if sh.firstSliceSegmentInPic {
		if !d.budget.take(int64(s.picWidthInLumaSamples) * int64(s.picHeightInLumaSamples)) {
			return nil, ErrUnsupported
		}
		prior := d.cur != nil
...
// ppsForSlice reads far enough into a slice segment header to find the picture
// parameter set it names, and whether it starts a picture.
