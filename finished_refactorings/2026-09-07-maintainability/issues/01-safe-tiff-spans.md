...
- [x] Check root, nested and next-IFD offsets, entry/value overflow, near-end spans, truncation and MaxUint32 in both byte orders before slicing.
- [x] Valid metadata, orientation and RAW embedded previews retain their current results and tolerant error policy; reader and writer malformed-block policies stay distinct.
- [x] Add the specified FuzzTIFFReaders target with deterministic seeds for the affected reader entry points; record the reproduced failure before the fix and the bounded fuzz result.
...
Implemented checked uint64 spans in metadata, orientation and RAW TIFF readers, preserving tolerant partial reads and the separate writer policy. Root-offset regression reproduced 12 panics; an original-source overlay also failed the nested-pointer guard. `go test ./internal/imaging -count=1` and focused vet passed. `FuzzTIFFReaders` passed 31.457s with 3,181,251 executions. `make verify` passed, including all 617 sharded UI tests and the Linux/amd64 Docker race suite. Lead standards/spec review found no remaining issue in this slice. No commit was made.
