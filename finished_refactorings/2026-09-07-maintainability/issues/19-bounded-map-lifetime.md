...
- [x] Concurrent warm passes and foreground requests share one declared per-fetcher limit rather than per-pass independent caps.
- [x] Bound queued work as well as active requests so per-URL goroutines or cancelled waiters cannot accumulate without limit; preserve useful request deduplication under admission pressure.
- [x] Superseded warm work and window closure cancel obsolete queued/in-flight work with observable worker completion.
- [x] Failure metadata has a tested capacity or expiry bound; retries and deduplication remain correct.
- [x] Use a controlled local tile server to verify bounds, cancellation, retries and callback behavior; never load public tile services.
- [x] Wait for onChange's own observable effect rather than a pending counter cleared before that callback.
...
Implemented and lead-reviewed. Full EXIF race suite passes (4.695s), viewer EXIF/shutdown selection passes (18.720s), and 15 negative mutations are rejected. Common 19–20 gate remains pending. The separate upstream decoded cache limitation is recorded in todos; this ticket bounds PicFetch requests, queued work, encoded cache bytes and failure state.
