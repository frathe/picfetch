...
- [x] Capture the current matrix for comparison, Copy Selection mode, Grid View, inspect and picture-frame mode before consolidating.
- [x] Test Escape priority, menu/keyboard/direct-entry parity and intentional exceptions, including comparison Help and Open refusal.
- [x] Centralize only repeated decisions when that reduces duplication; preserve feature state ownership and explicit composition.
- [x] Retain a tested matrix without forcing consolidation where it does not simplify the touched routes; do not reopen the separate chooser-thread defect.
...
Activated by the user’s 2026-09-07 request; matrix captured before consolidation.

Phase 6 completion: [evidence](../evidence/30-command-matrix.md) and the [full common gate](../evidence/phase6-verify.txt) pass. Lead review and eight negative overlays are complete. Windows native package/WACK validation remains deferred in ticket 27.
