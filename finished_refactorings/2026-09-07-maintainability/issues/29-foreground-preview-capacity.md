...
- [x] Start only when ticket 28 demonstrates contention and records a concrete foreground-latency target and chosen bounded policy.
- [x] Foreground work retains capacity under a cold favorite; compare actual before/after latency as well as throughput with the same workload.
- [x] Disk-preview generation converges when idle, cancellation does not sweep unvisited valid previews, and full-memory-cache behavior stays bounded.
- [x] Preserve separate foreground/background ownership; avoid a shared semaphore that lets previews starve visible thumbnails.
- [x] If measurement supports retaining the policy, record this ticket as intentionally unactivated rather than pretending a scheduler change was delivered.
...
Activated by ticket 28’s P2 contention measurement; bounded policy and targets are recorded in Phase 6 of the active plan.

Phase 6 completion: [evidence](../evidence/28-29-preview-contention.md) and the [full common gate](../evidence/phase6-verify.txt) pass. Lead review and eight negative overlays are complete. Windows native package/WACK validation remains deferred in ticket 27.
The retain-without-change alternative is not applicable: measured contention activated and verified the bounded policy.
