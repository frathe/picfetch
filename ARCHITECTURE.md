...
| `internal/ui/mosaicwin/` | Dedicated configuration/preview window with immutable command-entry sources/topology, accessible controls and focus order, distinct labels for identically named displays while preserving target IDs, cancellable generation lifecycle, stale-result rejection, exact-result export, targeted wallpaper callback, and remembered geometry. It is a secondary window, not a main-window overlay. | 4-method `Host` (`GenerateMosaic`, `InspectMosaicDisplays`, `SetMosaicWallpaper`, `AfterFileExported`). |
...
| `sync.go` | Cancellable `Sync` walk: memory → disk → decode, then `Sink`; waits all admitted work and skips sweep on cancellation. |
...
resampler/mask storage and bounded extra SVG rasterization. Tiles preserve the
original affine geometry and use clipped floating-point coverage. Rotated masks
