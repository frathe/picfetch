...
Ranked filtering and visit capture/restoration share a generation-bound source
index, so repeated result updates inspect only their bounded ranked identities.
...
| `wallpaper.go` | Shared ordinary/mosaic wallpaper lifecycle: serialize work, export captured pixels to a global or hashed-target cache scope, recheck a selected mosaic target and derive its single-display authorization after PNG encoding, call `wallpaper.Set(Request)`, and sweep only copies no longer backing another scope. |
...
| `await.go` / `await_other.go` | `AwaitPIDEnv` (`PICFETCH_UPDATE_AWAIT_PID`) relaunch handshake. `CleanupPredecessor`, called from `main.go` before `app.NewWithID`, waits (bounded, 15s) for the process that installed this executable to exit before preferences are touched, then sweeps legacy `<dest>.new` / `<dest>.apply.cmd` leftovers. On Unix it also streams the executable and macOS plist directories to remove regular randomized update siblings older than five minutes; fresh files and links are kept. `SweepBackup`, called from `internal/ui` startup once the Fyne app cache exists, removes `<dest>.old` — skipped when the last recorded apply failure has `Op == "restore"`, the one state where the backup is the user's only intact executable. |
...
Bounded demand-started workers + per-key in-flight claim. A lifecycle-bound
`Queue` removes its pending jobs on cancellation and invokes their callbacks
with failed admission; it keeps current interactive jobs ahead of low-priority
background work without creating a goroutine per queued source. Grid thumbs:
`Pool[*fyne.Container, thumbClaim]`. Display preloads: `Pool[string, struct{}]`.
