...
| `jpegprivacy.go` | `InspectJPEGMetadata`, its clean/removable/unsupported result, complete marker/scan removal policy, bounded decoding, JFIF/Adobe reconstruction and strict orientation extraction shared by inspection and the serialized mutation. |
| `jpegmemory.go` | Header-only admission for the metadata-removal working-memory budget: encoded copies, padded component planes, progressive coefficients, orientation and output validation; shared cancellable marker-fill traversal for admission and complete removal parsing. |
