...
| `internal/ui/mosaicwin/` | Dedicated configuration/preview window with immutable command-entry sources/topology, accessible controls and focus order, distinct labels for identically named displays while preserving target IDs, cancellable generation lifecycle, `progress.go` coalesced coverage updates on the tracked worker/UIQueue, a determinate coverage bar, stale-result rejection, exact-result export, targeted wallpaper callback, and remembered geometry. It is a secondary window, not a main-window overlay. | 4-method `Host` (`GenerateMosaic`, `InspectMosaicDisplays`, `SetMosaicWallpaper`, `AfterFileExported`). |
...
existing layout/render pass; only successful final composition reports completion. Rotated masks
