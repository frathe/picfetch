...
### Generated-license anchor finding on `0410e62`

Codex [reported](https://github.com/frathe/picfetch/pull/22#discussion_r3999689813)
that the introduction hard-coded an Apache text hash, allowing a dangling link
after the last matching license text changed. Lead confirmed the defect with
the existing synthetic inventory: the new link-to-heading invariant failed on
the absent `cfc7749b96f6` heading. Removing the redundant introductory sentence
fixes it; each module already links every applicable text by its actual hash.

Route: Standard follow-up in the same inventory seam. Lead changes the existing
generator/test, regenerated notices, `todos.md` and this record. No new dependency,
test file or interface. The regression passes after the removal, and the real
document check confirms all updater-license anchors resolve. Comparing original
and regenerated bytes confirms only that one explanatory sentence was removed;
all license text and per-module source/attribution entries are unchanged.

Focused race tests pass for updaternotices (3.241s) and msixstage (6.214s), plus
focused vet, formatting and Qodana exclusions. The changed regression and source
inventory check pass again after removing GoLand's redundant regex-escape warning.
Both changed Go files have clean GoLand inspections, including weak warnings.
